...
export default function Works() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const user = "your-github-username"; // замени
        const repos = await getRepos({ user, perPage: 12 });
        const works = repos.map(mapRepoToWork);
        if (!canceled) setItems(works);
      } catch (e) {
        if (!canceled) setError(e.message || "Ошибка загрузки");
      } finally {
  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-16" id="works">
      <header className="mb-8">
        <h2 className="text-2xl md:text-3xl font-semibold">Works</h2>
        <p className="mt-2 text-white/70">Последние проекты с GitHub</p>
      </header>

      {loading && <p className="text-white/70">Загружаю…</p>}
      {error && <p className="text-red-400">{error}</p>}

      {!loading && !error && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((w) => (
            <WorkCard key={w.id} work={w} variant="filled" size="md" animated />
          ))}
        </div>
      )}
    </section>
